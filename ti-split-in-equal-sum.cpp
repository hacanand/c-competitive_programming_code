...
void arrPartitionTillCurr(vector<int>&arr,int n,vector<bool>&curr_elements,int no_of_selected_elements,vector<int>&soln,int &min_diff,int sum,int curr_sum,int curr_position){
    if(curr_position==n)
        return;
    if((n/2-no_of_selected_elements)>n-curr_position)
        return;
...
{
    int n = arr.size();
    vector<int> curr_elements(n, false);
    vector<int> soln(n, false);
    int min_diff = INT_MAX;
    int sum = 0;
    for (int i = 0; i < n;i++){
        sum += arr[i];
